...
v.Page("/counter/{id}", func(c *via.Composition) { // Composition = page definition
    ...
    increment := via.Action(c, func(s *via.Session) { // Session = runtime context
...
- `*CompHandle` - Handle returned by `c.Component()` with `Mount(s *Session) h.H` method
...
**Creating action handlers:** `via.Action(c, func(s *via.Session) { ... })`, attach with
