...
- When exploring the codebase or reading any file, read [CONVENTIONS.md](./CONVENTIONS.md)
  ...
  in the file.
- Do not commit without explicit user approval. Ask the user before committing.

## When a property becomes a duck-typed method

via's duck-typed surface is deliberately tiny — `View`, `OnInit`, `OnReload`,
`Title` — in two families: lifecycle phases, and document queries. Every
...
`Title` clears all three, which is why it is a method on the root and not
`ctx.Title(…)`: the `<title>` is one shared slot, and the setter shape let any
embedded child silently rename the page.
...
There is deliberately no `Description` hook. If a second document slot is ever
needed, the answer is one `Meta() via.Meta` hook that subsumes `Title`, not a
fifth sibling method.
