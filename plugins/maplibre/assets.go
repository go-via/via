...
// The vendored MapLibre GL JS dist files. Refresh with `go generate`
// (see refresh_assets.sh); bump pinnedVersion in plugin.go in the same
// change.
//
//go:embed assets/maplibre-gl.js assets/maplibre-gl-csp.js assets/maplibre-gl-csp-worker.js assets/maplibre-gl.css
var embeddedDist embed.FS

const assetPathPrefix = "/via/assets/maplibre/"
...
func mustSameOriginURL(origin, url string) {
	...
	if strings.HasPrefix(url, "http://") || strings.HasPrefix(url, "https://") ||
		...
		panic(origin + ": cross-origin URLs need Subresource Integrity — " +
			"use WithCDN/WithCDNStylesheet(url, integrity)")
	}
}
