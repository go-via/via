...
func TestUserHandle_TypeExists(t *testing.T) {
	...
	v.Page("/", func(c *Composition) {
		...
		c.View(func(s *Session) h.H { return h.Div() })
	})
}
...
func TestUserHandle_IsAuthenticated_False(t *testing.T) {
	...
	assert.False(t, userHandle.IsAuthenticated(s))
}
...
func TestUserHandle_Get_AfterSet(t *testing.T) {
	...
	if v.sessionState[s.sessionID] == nil {
		v.sessionState[s.sessionID] = make(map[string]any)
	}
	v.sessionState[s.sessionID][userHandle.id] = TestUser{ID: "123", Name: "Alice", Role: "admin"}
	v.sessionStateMu.Unlock()

	// Get should return user and true
	user, ok := userHandle.Get(s)
	assert.True(t, ok)
...
func TestUserHandle_IsAuthenticated_AfterSet(t *testing.T) {
	...
	v.sessionState[s.sessionID][userHandle.id] = TestUser{ID: "123", Name: "Alice"}
	v.sessionStateMu.Unlock()

	assert.True(t, userHandle.IsAuthenticated(s))
}
