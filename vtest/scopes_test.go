...
func TestSessionScopes_Example(t *testing.T) {
	...
	v.Page("/", func(c *via.Composition) {
		...
		incTab := via.Action(c, func(s *via.Session) {
			tabCounter.Set(s, tabCounter.Get(s)+1)
		})

		incApp := via.Action(c, func(s *via.Session) {
			appVisits.Set(s, appVisits.Get(s)+1)
		})

		setToken := via.Action(c, func(s *via.Session) {
			sessionToken.Set(s, "auth-token-123")
		})

		login := via.Action(c, func(s *via.Session) {
			user.SetUser(s, User{ID: "user-1", Name: "Alice", Role: "admin"})
			s.Sync() // Sync to update the view
		})

		logout := via.Action(c, func(s *via.Session) {
			user.Logout(s)
			s.Sync() // Sync to update the view
		})

		c.View(func(s *via.Session) h.H {
			u, authenticated := user.Get(s)
...
func TestSessionScopes_CookiePersistence(t *testing.T) {
	...
	v.Page("/", func(c *via.Composition) {
		c.View(func(s *via.Session) h.H {
			return h.Div(
				h.P(h.Textf("Session: %s", s.SessionID())),
			)
		})
	})
