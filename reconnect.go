...
//     frozen forever). Reload to re-bootstrap a fresh stream, after a jittered
//     delay so a fleet of tabs doesn't stampede the new pod at once.
//
...
const reconnectInit = `(()=>{if(window.__viaRC)return;window.__viaRC=1;` +
	...
	// so the patch kinds must be matched here or the banner sticks forever and
	// its full-width overlay swallows clicks.
