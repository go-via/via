	...
	return []glossaryTerm{
		...
		{"CSP",
			[]h.H{s("The Content-Security-Policy header via sends with each document. It is derived once per mount from the router's "), API("via.WithHead"),
				s(" assets and the page's Meta.Assets, and admits via's own inline scripts by hash. "), Code("script-src"), s(" always carries "), Code("'unsafe-eval'"),
				s(" for Datastar's expressions.")},
