...
func (p *Helpers) View() h.H {
	...
	return d.Page(
		...
		h.P(API("h.Href"), h.Str(", "), API("h.Src"), h.Str(" and "), API("h.Action"),
			h.Str(" admit relative URLs, http: and https:, and nothing else. Any other scheme, a protocol-relative "),
		...
		Callout(Caveat, "mailto: and tel: are refused",
			h.P(h.Str("The allowlist is http and https, with no option to widen it, and "), API("h.RawAttr"),
				h.Str(" applies the same check. h cannot render a mailto: or tel: link.")),
		),
