...
var composition = []row{
	...
	{`via.Mount(r, "/thread/{id}", Thread{})`,
		"Registers a page composition at an http.ServeMux pattern. root is taken by value, and the page's actions post to {path}/_via/a/{child}/{act}."},
...
var options = []row{
	...
	{"via.WithPinnedDeadline(d)",
		"How long an action POST waits for the tab's stream goroutine before answering 503 and logging the tab as pinned (default 5s)."},
