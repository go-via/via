...
const tsChildPointer = `panic: via: found at Mount, rendering *main.Page as mounted: via: via.Child(child) requires child to have a View() method`
...
const tsV07Connect = `level=WARN msg="via: main.Room.OnConnect is shaped like the v0.7 OnConnect hook, which v0.8 never calls. Move its work into OnInit: ctx.OnConnect(fn) runs fn once when the stream opens, and ctx.Tick or ctx.Listen makes the unit live."`
...
503 stream busy
level=WARN msg="via: tab pinned — its stream goroutine is inside a Tick, Listen, OnConnect or action handler, so its keepalives have stopped too and every action on this tab answers 503 until it returns. Move blocking work off the handler."`

const tsBlocked = `Executing inline script violates the following Content Security Policy directive 'script-src 'self''. Either the 'unsafe-inline' keyword, a hash ('sha256-…'), or a nonce ('nonce-...') is required to enable inline execution.
Uncaught Error: Blocked by CSP.`
...
func (p *Troubleshooting) View() h.H {
	...
	return d.Page(
		...
		h.P(API("via.WithTrustedOrigin"), h.Str(" turned the origin check on and the browser's request fails it. "+
			"The allowlist compares scheme, host and port, so the usual misses are:")),
		h.Ul(
			h.Li(h.Str("A different spelling: "), Code("http://"), h.Str(" for "), Code("https://"), h.Str(", "),
				Code("www."), h.Str(", a non-default port.")),
		...
		h.P(h.Str("The browser reloads the tab on its own after a 410, with a \"Page is out of date - reloading…\" banner. "+
			"An action for a live tab whose stream is still connecting, or reconnecting, waits up to 2 seconds (less if "),
			API("via.WithPinnedDeadline"), h.Str(" is shorter) for it, and answers 410 only if the stream has not come by then. "+
				"When via ended the stream itself, the 410 comes at once.")),
		...
		table([]string{"Banner", "Cause", "What happens next"},
			...
			[]h.H{h.Str("Disconnected., and it stays"), h.Str("The stream's connect answered 403; or two automatic reloads already; or 20 probes failed."),
				h.Str("Nothing until you press Reconnect, which clears the reload count.")},
		),
		h.P(h.Str("Every banner but Reconnecting… carries a Reconnect button that probes at once. An action that fails "+
			"with the stream still up (a 500 from a handler panic, a 503, a 403) shows no banner and leaves "),
			Code("data-via-connection"), h.Str(" alone; Datastar fires a "), Code("datastar-fetch"),
			h.Str(" error event on the element that made the request, for a message of your own.")),
		h.P(h.Str("A banner right after every deploy is expected; state that lives only in a "), API("via.State"),
			h.Str(" starts over, since a reconnect is a new connection. A Disconnected banner that stays on first load is a stream "+
				"connect answering 403, or a 5xx on every reload: read the Network tab for the status and find it on this page. The reload count clears 5 seconds after a "+
		...
		h.P(h.Str("Besides "), Code("View"), h.Str(" and "), Code("PageMeta"), h.Str(", v0.8 calls two methods: "),
			...
			h.Str(" method still compiles and is never called. Mount warns once about the OnConnect; the OnDispose has an "+
				"action's shape, so via says nothing. Register both from "), Code("OnInit"), h.Str(" with "), API("via.Ctx.OnConnect"),
		...
		snippet.Text("log", tsV07Connect),
		h.P(h.Str("A method named like a hook with the hook's exact signature, on a type that lacks the hook, warns once. "+
			"That covers aliases (Init, Reload, Refresh, Metadata) and names one typo from a hook (OnRelaod, Oninit); "+
			"OnRelay and OnInput are left alone.")),
		...
		h.P(API("via.Mount"), h.Str(" also renders the mounted value once, without running "), Code("OnInit"),
			h.Str(", and panics on a wiring mistake that render reaches: a func literal bound once per row, a method taken "+
				"through an interface field or an ambiguous value receiver, "), Code(`h.El("script")`),
			h.Str(", a Signal with no slot, a child without a View. Those messages start "), Code("found at Mount"),
			h.Str(". Any other panic in that render is ignored, since the page has no data yet. A mistake behind a branch "+
				"the empty value does not take panics at the first render that takes it.")),
		d.H3("via.Child given a pointer"),
		snippet.Text("panic", tsChildPointer),
		...
		h.P(h.Str("A composition mistake that only a render with data can see, or that Mount's render did not reach. The body is always "), Code("render failed"),
		...
		h.P(Code("stream busy"), h.Str(" is one tab's goroutine stuck in a handler for longer than "), API("via.WithPinnedDeadline"),
			h.Str(" (5 s). Tick, Listen and actions on a tab run one at a time, so a blocking call in any of them stalls the "+
				"rest. Do slow work in a goroutine and publish the result back through a topic. When the line says "),
			Code("blocked writing a frame to a client that is not reading"),
			h.Str(" instead, no handler is at fault: the browser stopped reading, and the stream closes once the 10 s "+
				"write deadline passes. A 503 with "),
			Code("session store unavailable"), h.Str(" is a "), API("via.SessionStore"), h.Str(" that failed to load.")),
		tsSee(d, "/security#request-limits", "Sessions & security › Request limits"),

		d.H2("Error: Blocked by CSP"),
		snippet.Text("console", tsBlocked),
		h.P(h.Str("Datastar compiles each expression by adding a script that carries the page's nonce, and via's own policy admits "+
			"it. The error means a second policy reached the browser, from middleware or the proxy (nginx "), Code("add_header"),
			h.Str(", Caddy "), Code("header"), h.Str("), and browsers enforce every policy they receive. Remove the extra "),
			Code("script-src"), h.Str(", or build it from via's header so it carries the same nonce; "), Code("'unsafe-eval'"),
			h.Str(" does not help. Asset origins belong in "), APIText("via.Meta", "PageMeta().Assets"),
			h.Str(", which via folds into its own policy.")),
		d.H3("EvalError: Refused to evaluate a string as JavaScript"),
		snippet.Text("console", tsEval),
		h.P(h.Str("A script of yours, or a library it loads, calls "), Code("eval"), h.Str(", "), Code("new Function"),
			h.Str(" or "), Code("setTimeout"), h.Str(" with a string. via's policy forbids it. Use a build of the library "+
				"that does not evaluate strings, or, if there is none, set "), API("via.WithUnsafeEval"),
			h.Str(", which allows it on every page.")),
