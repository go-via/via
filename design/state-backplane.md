...
- **New opt-in sibling: `StateAppLog[E, V]`.** For high-churn shared state (a
...
- **The backplane is one `Store + Log (+ optional Compactor) + Codec`
...
to a **durable, ordered, offset-resumable Log** dissolves stranding/reconnect
...
- Detection/binding: marker interfaces in `walker.go` (`isStateApp`…) →
  `bindScopeKeys` in `runtime.go`. `StateAppLog` gets its own marker so the
...
//             then Append a value-less Change to the Log) AND holds
//             StateAppLog fold-snapshots + side-effect consumer checkpoints.
//   - Log   : a durable, ORDERED, OFFSET-RESUMABLE append log. This is the
//             load-bearing half: because a pod always resumes from its last
//             committed Offset and CANNOT miss a record, the Log alone
...
//
// The Codec is supplied to the runtime PER StateAppLog[E] (see WithBackplane
...
// Rev is the Store cell's CAS version, DISTINCT from Offset (Store and Log
// have independent counters; a backend MAY alias them but need not).
type Rev uint64

// Record is one delivered Log entry. The runtime, not the backend,
// interprets Data via the per-key Codec — the backend only moves bytes and
// assigns Offset.
type Record struct {
	Key    string // the log subject (a StateAppLog wire key, or the shared "changes" feed)
	Offset Offset
...
type Store interface {
	...
	// ONE place value-shaped StateApp tolerates a retry; StateAppLog.Append
	// never CASes, which is exactly why it beats value-shaped state under
	// churn (open #2). Used for: value-StateApp survivability, StateAppLog
...
// Log is the durable, ordered, offset-resumable append log that REPLACES the
...
//
// EXPLICIT NON-GUARANTEES: NO cross-key ordering (distinct StateAppLog fields
...
type Log interface {
	...
	// point of the Log path (#2). Used by StateAppLog.Append AND by the
...
// Codec serializes events and snapshots. Supplied PER StateAppLog[E] (the
...
type Codec interface {
	...
	// via.log.undecodable metric — a poison/forward-incompatible record must
...
## Recommended user API — `StateAppLog[E, V]`

```go
// Package via — StateAppLog[E], an event-log-backed sibling of StateApp[T].
//
...
// — StateAppLog[E] holds an append-only sequence of immutable events E whose
...
// conflict (the Log orders them; there is no CAS to lose), the payload is
...
//
//	type Room struct {
//	    Log via.StateAppLog[ChatEvent, []Message] // events:ChatEvent, value:[]Message
//	}
//
...
// change.
type StateAppLog[E EventReducer[E, V], V any] struct {
	wireKey string
...
func (l *StateAppLog[E, V]) bindWireKey(k string) { l.wireKey = k }
func (l *StateAppLog[E, V]) bindApp(a *App)        { l.app = a }
func (*StateAppLog[E, V]) isStateAppLog()          {} // distinct marker → walker
//                                                       starts the projector only for log keys

// Key returns the wire key (lowercase field name unless overridden by `via:` tag).
func (l *StateAppLog[E, V]) Key() string { return l.wireKey }

// Read returns the current PROJECTED value: the fold of every event in the
// Log up to this pod's locally-applied offset, seeded by E.Zero(). A Read
...
// incrementally as Log records arrive (one tailer goroutine per key folds
// each Record forward). Read NEVER re-folds from genesis. Accepts *Ctx or *CtxR.
func (l *StateAppLog[E, V]) Read(rc readCtx) V {
	var ev E
	zero := ev.Zero()
	if rc == nil || l.app == nil {
		return zero
...
// Append commits ONE immutable event to the Log. Unlike StateApp.Update there
...
// conflict (the Log orders them; there is no CAS to lose), so the chat/ticket
...
// via autoflush), and every other pod tailing the Log from its offset
...
func (l *StateAppLog[E, V]) Append(ctx *Ctx, ev E) (offset uint64, err error) {
	if ctx == nil {
		panic("via: StateAppLog.Append called with nil *Ctx")
	}
	if l.app == nil {
		return 0, nil // nil backplane pre-Mount: parity with StateApp's no-op guard
	}
	off, err := l.app.appendEvent(l.wireKey, ev) // Codec.Encode + Log.Append + local fold
	if err != nil {
		return 0, err
	}
	ctx.markStateDirty()
...
// plain Append.
func (l *StateAppLog[E, V]) AppendIf(ctx *Ctx, expectedHead uint64, ev E) (offset uint64, err error)
...
func (l *StateAppLog[E, V]) ReadAt(rc readCtx) (V, uint64)

// Text returns the projected value as a text node. Sibling of StateApp.Text.
func (l *StateAppLog[E, V]) Text(rc readCtx) h.H { return h.Textf("%v", l.Read(rc)) }

// OnEvent registers a NAMED, offset-tracked, side-effecting consumer over the
// Log (send email on ticket-closed, charge a card). Side effects do NOT live
...
func (l *StateAppLog[E, V]) OnEvent(name string, fn func(ctx *Ctx, ev E, off uint64) error)
...
// StateAppLog: append a Tick. Appends are ordered, never conflict.
...
type Room struct {
	Log   via.StateAppLog[ChatEvent, []Message] // was StateAppSlice[Message]
...
func (r *Room) Send(ctx *via.Ctx) {
	...
	_, _ = r.Log.Append(ctx, ChatEvent{Kind: "say", Msg: Message{From: name, Body: body}})
	r.Draft.Write(ctx, "")
}
...
func (r *Room) View(ctx *via.CtxR) h.H { // identical to the existing example
	return h.Main(h.Class("container"),
		...
		h.Article(h.Each(r.Log.Read(ctx), func(m Message) h.H {
...
Grounded in code: StateApp/StateSess detect via marker iface in walker.go (isStateApp/isStateSess → roleStateApp); bind via scopeBinder.bindWireKey in runtime.go:bindScopeKeys. Gave StateAppLog its OWN marker isStateAppLog() + a new roleStateAppLog (NOT versioning's "reuse isStateApp marker") so the walker starts the per-key projector goroutine only for log keys and scopeSlot carries a kind flag — minimal additive wiring, no conflation of append-mode detection. Read reuses ctx.trackRead/subscribed verbatim (ctx.go); Append reuses broadcastRender→go SyncNow→Read (broadcast.go, confirmed path) and panic-on-nil-ctx parity with StateApp.Update (stateapp.go:60).

Kept V as the 2nd type param despite the uglier `StateAppLog[ChatEvent,[]Message]` decl — collapsing it (adapters/versioning erase View to `any`) loses compile-time projection safety and the counter→int / chat→[]Message demo is the core pitch. The decl cost is paid once per field.

Borrowed from the other lenses: adapters' OPTIONAL Compactor (type-asserted, backend declines) — honest about the Kafka leak vs api's core Log.Compact; adapters' snapshot=pure-cache / invalidate-on-codec-hash → re-fold (so evolving V is FREE, no snapshot upcasters). versioning's drop-on-undecodable (ErrUndecodable → fold no-ops, never panics the pod) + stable TypeTag decoupled from Go type name — load-bearing for a poison event not wedging the cluster. api/correctness' shared "changes" subject for value-shaped fan-out + per-key subjects for logs.
...
Structural: StateAppLog.Append → Log.Append (plain, no expected-offset, never conflicts). N concurrent appends from N pods all land, totally ordered by the Log's per-key monotonic counter. No shared value cell to CAS → nothing to retry-storm. Throughput bounded by per-key append rate, not CAS amplification.

### #3 stranding — RESOLVED

Durable+ordered+resumable Log. Pod resumes Subscribe(key, from=lastAppliedOffset); backend replays the gap then live-tails. Next Read folds the replayed gap first → cannot be stranded with a stale projection. No census, no re-broadcast needed.

### #4 Broadcast/BroadcastSignals cross-pod count — RESOLVED

Broadcast/BroadcastSignals STAY LOCAL — int return documented as live-tab count on THIS pod (broadcast.go already iterates snapshotContexts; no signature break, v0.4.0 safe). Cluster announce: add app.BroadcastCluster(script) that Appends a Broadcast control-event to the shared changes feed; every pod tails it, runs its local Broadcast; returns (queued bool, err), NOT a synchronous global count (a sync global count needs a blocking scatter-gather census). Exact count only via app.ClusterTabCount(ctx) — explicitly best-effort, RTT-bounded, from heartbeat keys. CUT both cluster helpers from v1 (orthogonal to StateAppLog; local Broadcast already works).
...
**User API shape**
- api → `StateAppLog[E EventReducer[E,V], V any]`. Two type params; fold is METHOD on E (compile-bound). Read=method `Read(rc) V`. Most type-safe, but `[E,V]` double-param ugly at field decl (`StateAppLog[ChatEvent,[]Message]`).
- correctness → `StateAppLog[E]` field, V inferred from a `Fold(P,E)P` method on a SEPARATE composition-level `StateFolder` iface, checked at Mount. Clean field decl; fold lives off the event type → can capture (impurity hole vs api).
- adapters → `StateAppLog[E]` field + `via.Project(&l, foldClosure)` in OnInit + free fn `via.ReadLog[V](&l, ctx)`. Closure fold = MOST impurity-prone; ReadLog free-fn is ugly call site, breaks "sibling of StateApp" feel.
- versioning → `StateAppLog[E]` + `via.OnFold(ctx,&l,seed,reduce)` in OnInit; `View` type erased to `any`, Read returns `View`. Cleanest E-only decl; explicit View-vs-E split named in types. Closure fold (impure-prone) but OnFold-in-OnInit idempotent guard.
...
- AppendIf (optimistic expected-offset, claim-ticket): api keeps + `ReadAt`→(V,head); correctness keeps `AppendExpecting`+sugar `AppendIfStable` retry-loop; adapters CUTS to v1; versioning omits. Backend prim split: api→`Log.AppendIf`; correctness/versioning→`Log.AppendExpecting`/CAS-on-Store.
...
- Compact placement: api/correctness/versioning → `Log.Compact` (core). adapters → SPLIT into OPTIONAL `Compactor` iface, type-asserted, backend may decline (Kafka event-log can't native-compact). Best leak-honesty.
...
- ALL: ONE Backplane instance, value-shaped = CAS Store + value-LESS `Change{key,rev}` on Log → notify-and-pull (reuse broadcastRender→SyncNow→Read, confirmed path). Log-shaped = full events folded.
- Subject layout DIVERGES: api/correctness → value uses ONE SHARED change subject (`changes`/`app:change`), log uses PER-KEY subject. versioning → ONE stream per key for BOTH, discriminate by envelope TypeTag (`via.change` vs user tag). adapters → per-key for both, derived names `via.log.<k>`/`via.change.<k>`.
...
- ALL: mandatory versioned ENVELOPE `{t/typeTag, v, d}`; upcaster chain at DECODE only (never rewrite Log); additive-first discipline; tagged-struct (NOT sealed iface) so unknown variants decode; Fold `default:`→identity for forward-compat.
...
- PHASE 0 — binding seam (additive, no behavior change). Add isStateAppLog() marker + roleStateAppLog in walker.go classifyField; scopeSlot gains a kind flag (value vs log); bindScopeKeys binds StateAppLog via the existing scopeBinder path + a new bindApp. nil backplane everywhere → zero observable change. v0.4.0 typed API untouched. Tests: existing suite green + a nil-backplane in-process StateAppLog folding in RAM (today's StateAppSlice semantics).
- PHASE 1 — in-process StateAppLog[E,V] core. EventReducer constraint + Read (cached projection, O(1), trackRead) + plain Append (broadcastRender reuse, nil-ctx panic) + Text. App gains logProjection map + per-key incremental fold under lock. NO backplane yet — proves the user API and the fold/projection cache against the chat+counter call sites. Migrate internal/examples/chat to StateAppLog behind nothing (still single-pod), delete the trim-Update.
- PHASE 2 — Backplane interface + ONE reference backend (NATS JetStream+KV: durable ordered resumable Log AND CAS KV in one dependency, maps the interface most directly). Store(LoadSnapshot/CAS) + Log(Append/AppendIf/Subscribe/Head) + io.Closer + per-field Codec (default versioned-envelope JSON). WithBackplane Option. Cold start = LoadSnapshot→Subscribe(from:coveredOffset)→fold. Ship the backend CONFORMANCE TEST SUITE (ordering, gap-free resume, CAS conflict, snapshot-before-compact). This phase alone closes #3/#7.
...
- PHASE 4 — E-versioning hardening (#6). Versioned envelope MANDATORY (enforced at Mount: no TypeTag → fail fast), RegisterEvent[E](v, Upcaster{From,To}) decode-chain, drop-on-undecodable + via.log.undecodable metric, forward-incompat projector-halt guard. Snapshot=pure-cache, invalidate-on-codec-hash → re-fold (evolving V free). Additive-first godoc.
...
StateAppLog[E]: an event-log sibling of StateApp where Read is a pure fold over an append-only Log, killing the CAS retry-storm and making side-effects offset-once — the fold is a method on E's reducer, not a func field, so it's deterministic by construction
...
- CUT BroadcastCluster + ClusterTabCount from the initial cut — they're #4 nice-to-haves, orthogonal to StateAppLog, and the honest async-count story can land in a follow-up. Local Broadcast already works.
...
StateAppLog[E]: an append-only sibling of StateApp whose value is a deterministic fold over a durable, ordered, offset-resumable Log. Append events (never CAS), fold to project, Read subscribes the render exactly like StateApp.Read. One Backplane interface (Store+Log+Codec) backs NATS/Redis/PG/Kafka. The Log is the source of truth; the Store holds only fold-snapshots+offsets so cold start is replay-from-snapshot, not replay-from-zero.
...
- CUT app.Census / cross-pod broadcast count from this feature entirely. It's a real question (#4) but orthogonal to the Log; bundling it bloats scope. Document the local-count semantics honestly and ship Census separately if demand appears.
...
- CUT per-variant snapshot versioning cleverness: since V is always re-derivable from E, a snapshot is pure cache — version it with a single opaque codec hash and INVALIDATE-on-mismatch (re-fold from events). No snapshot upcasters, ever. Shrinks the Codec contract to envelope + event upcasters only.
...
- NON-NEGOTIABLE, stays: Append + Project + ReadLog, the Store+Log+Codec interface, envelope+upcaster versioning, snapshot+optional-Compactor, and nil-backplane == today's exact behaviour.

### versioning

StateAppLog[E] = sibling of StateApp that APPENDs immutable events and Reads a fold; both ride ONE Store+Log+Codec backplane. StateApp is the degenerate single-key compacting case of the same Log, so no special-casing. The forever-persistence of E is paid for by a mandatory versioned envelope + a codec that upcasts on decode, never on store — and a hard rule: a fold that can't decode an old event must DROP it, never panic the pod.
...
- what to cut: Cut for v1, in order: (1) Compaction/snapshot of StateAppLog — ship append+fold+full-replay first; backends that keep full history (Postgres seq table, Kafka log) work without it, and a counter/chat-of-the-day rarely outgrows RAM in the demo window. Snapshots are a scale optimization (#9-#10 territory), not correctness. (2) The shadow divergence verifier — nice canary, not needed to ship; document the law first. (3) ClusterBroadcast census — keep Broadcast local-only with corrected godoc; a real cross-pod broadcast is a separable follow-up. (4) Kafka and Postgres backends — ship ONE reference backend (NATS JetStream+KV: it gives durable ordered resumable Log AND CAS KV in one dependency, matching the interface most directly) to prove the Backplane interface, then add Redis, then the rest. (5) Multi-version upcaster CHAINS — v1 ships with single-step upcasters (vN→vN+1) only; if someone needs v1→v5 they register four steps. Do NOT cut: the Envelope+TypeTag+Version (retrofitting versioning onto an unversioned Log is impossible since records are immutable forever — this MUST be in the very first byte ever appended), drop-on-undecodable, the pure-reducer contract, and nil-backplane degradation.
